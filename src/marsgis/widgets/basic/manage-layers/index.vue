<template>
  <mars-dialog :min-width="320" bottom="40" right="10" title="图层" top="50" width="320">
    <mars-tree v-model:checkedKeys="checkedKeys" v-model:expandedKeys="expandedKeys" :tree-data="treeData" checkable @check="checkedChange">
      <template #title="node">
        <mars-dropdown :trigger="['contextmenu']">
          <span @dblclick="flyTo(node)">{{ node.title }}</span>
          <template v-if="node.hasZIndex" #overlay>
        ...
        <span v-if="node.hasOpacity" v-show="node.checked" class="tree-slider">
          ...
          <a-slider v-model:value="opacityObj[node.id]" :max="100" :min="0" :step="1" @change="opcityChange(node)"/>
        </span>
      </template>
    </mars-tree>
  </mars-dialog>
