<template>
  <mars-pannel :before-close="beforeClose" bottom="10" closeable right="10" top="10" width="400">
    <a-form :label-col="{ span: 6 }" :model="formState" :rules="rules" :wrapper-col="{ span: 18 }">
      <a-collapse v-model:activeKey="activeKey">
        ...
        <a-collapse-panel key="1" header="表单控件">
          ...
          <a-form-item label="滑动条">
            <a-slider v-model:value="formState.brightness" :max="1.5" :min="-0.5" :step="0.05" @change="onSliderChange"/>
          </a-form-item>

          <a-form-item label="刻度滑动条">
            <a-slider v-model:value="formState.contrast" :marks="marks" :max="255" :min="-255" :step="1" @change="onMarkSliderChange"/>
          </a-form-item>
        ...
        <a-collapse-panel key="3" header="树控件">
          <mars-tree v-model:checkedKeys="checkedKeys" v-model:expandedKeys="expandedKeys" :tree-data="treeData" checkable @check="onCheckTreeItem">
...
import { $alert, $hideLoading, $message, $notify, $showLoading } from "@mars/components/mars-ui/index"
